...
test('base repository can be instantiated', function (): void {
    $abstract = Laravelplus\RepositoryPattern\BaseRepository::class;
    expect(class_exists($abstract))->toBeTrue();
});

test('base repository CRUD methods can be called', function (): void {
...
test('base repository hides fields', function (): void {
    $model = Mockery::mock(Illuminate\Database\Eloquent\Model::class);
    $model->shouldReceive('getTable')->andReturn('test_table');
...
test('MultiDatabase trait runs callback on connection', function (): void {
    $trait = new class()
    {
...
test('Cacheable trait caches all results', function (): void {
    $model = Mockery::mock(Illuminate\Database\Eloquent\Model::class);
    $model->shouldReceive('getTable')->andReturn('test_table');
...
test('SoftDeletes trait soft deletes and restores', function (): void {
    $model = Mockery::mock(Illuminate\Database\Eloquent\Model::class);
    $model->shouldReceive('getTable')->andReturn('test_table');
    $model->shouldReceive('where')->with('id', 1)->andReturnSelf();
    $model->shouldReceive('update')->with(Mockery::on(fn ($arg) => is_array($arg) && array_key_exists('deleted_at', $arg)))->andReturn(true);
